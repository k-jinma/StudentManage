...
public class StudentManager {
	...
	public void addStudent() {
		...
		try {
			...
			System.err.println("正しい年齢を入力してください。");
			...
			if (dbId.isEmpty()) {
				...
				case "6":
					...
					do {
						...
						input = sc.nextLine();
						if( input.equals("n") ) {
							System.out.println("処理をキャンセルします");
							return;
						}
					}while( !input.equals("y") );
					
					String deleteSql = "DELETE FROM shiken WHERE id = ?";
	...
	public void deleteTest() {
		...
		try {
			...
			while (rs.next()) {
				...
				System.out.printf("%-15s",rs.getString("subject_name"));
				System.out.printf("%-5d\n",rs.getInt("subject_no"));
			}
			System.out.println("-----------------------------------------");
